...
final class ExceptionHandlerAdvice {
	...
	public RegistryError handle(@NotNull final InvalidRepositoryNameException irne) {
		return new RegistryError(RegistryErrorType.NAME_INVALID, irne.getRepositoryName());
	}
	
	@ExceptionHandler(value = InvalidDigestException.class)
	public RegistryError handle(@NotNull final InvalidDigestException ide) {
		return new RegistryError(RegistryErrorType.DIGEST_INVALID, ide.getDigest());
	}
	
	@ExceptionHandler(value = InvalidTagException.class)
	public RegistryError handle(@NotNull final InvalidTagException ide) {
		return new RegistryError(RegistryErrorType.TAG_INVALID, ide.getTag());
	}
	
	@ExceptionHandler(value = InvalidBlobUploadException.class)
	public RegistryError handle(@NotNull final InvalidBlobUploadException ibue) {
		return new RegistryError(RegistryErrorType.BLOB_UPLOAD_INVALID, ibue.getUploadId());
	}
}
